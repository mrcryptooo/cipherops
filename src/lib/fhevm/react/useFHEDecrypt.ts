"use client";
// Source: packages/fhevm-sdk/src/react/useFHEDecrypt.ts (zama-ai/dapps)
// Adapted: removed .js extensions; kept all debug console.log statements
// as they are useful for Phase 2 debugging.
  ...
  const decrypt = useCallback(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        if (isStale()) { setMessage("Decrypt cancelled (stale)"); return; }
        ...
        try {
          ...
          const code = err?.name ?? "DECRYPT_ERROR";
          const msg = err?.message ?? "Decryption failed";
          setError(`${code}: ${msg}`);
          setMessage("userDecrypt failed");
          return;
        }

        if (isStale()) { setMessage("Decrypt cancelled (stale)"); return; }
        setResults(res);
        setMessage("Decrypt complete");
      } catch (e) {
        const err = e as { name?: string; message?: string };
        const code = err?.name ?? "UNKNOWN_ERROR";
        const msg = err?.message ?? "Unknown error";
        setError(`${code}: ${msg}`);
        setMessage("Decrypt errored");
      } finally {
