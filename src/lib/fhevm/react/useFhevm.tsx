"use client";
// Source: packages/fhevm-sdk/src/react/useFhevm.tsx (zama-ai/dapps)
// Adapted: removed .js extensions.
...
export function useFhevm(parameters: {
  ...
  useEffect(() => {
    if (_isRunning === false) {
      if (_abortControllerRef.current) { _abortControllerRef.current.abort(); _abortControllerRef.current = null; }
      _setInstance(undefined); _setError(undefined); _setStatus("idle");
      return;
    }
    if (_isRunning === true) {
      ...
      _assert(!_abortControllerRef.current.signal.aborted, "controller not aborted");
      _setStatus("loading");
      _setError(undefined);
      ...
      createFhevmInstance({
        ...
        .then((i) => {
          ...
          _setInstance(i); _setError(undefined); _setStatus("ready");
        })
        .catch((e: unknown) => {
          if (thisSignal.aborted) return;
          _assert(thisProvider === _providerRef.current, "thisProvider === _providerRef.current");
          _setInstance(undefined); _setError(e as Error); _setStatus("error");
        });
    }
