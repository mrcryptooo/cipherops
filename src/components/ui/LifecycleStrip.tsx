// All 5 lifecycle steps are now live — step 5 "Operate" includes Disperse/Airdrop/Vesting.
...
const STEPS = [
  ...
  { n: 5, label: "Operate",   detail: "Disperse · Airdrop · Vest" },
];
        ...
        <div className="flex flex-wrap items-start justify-center py-8 sm:flex-nowrap sm:items-stretch">
          {STEPS.map((step, i) => (
            <div key={step.label} className="flex items-center">
              ...
              {/* Connector — yellow gradient for all live steps */}
              {i < STEPS.length - 1 && (
                <div
                  className="hidden h-px w-6 flex-shrink-0 sm:block"
                  style={{
                    background: "linear-gradient(to right, rgba(255,210,8,0.40), rgba(255,210,8,0.15))",
                  }}
                />
              )}
            </div>
